...
export const runFeedback = async (
  ...
  console.log("toHistory in RUN FeedBack ->", toHistory);
  ...
  if (firebase.status) {
    const historyAnswer = saveHistory(
      pkg4History,
      history,
      toHistory,
      lastValue,
...
const updateLastValue = (profileID, data, lastValue, toLastValue) => {
  ...
  let prevLV = {};
  Object.assign(prevLV, newLastValue[profileID]);
  let currLV = {};
  Object.assign(currLV, newLastValue[profileID]);

  currLV.kitchenHot =
    data.kitchenHot !== HISTORYEMPTY ? data.kitchenHot : prevLV.kitchenHot;
  currLV.kitchenCold =
    data.kitchenCold !== HISTORYEMPTY ? data.kitchenCold : prevLV.kitchenCold;
  currLV.bathHot =
    data.bathHot !== HISTORYEMPTY ? data.bathHot : prevLV.bathHot;
  currLV.bathCold =
    data.bathCold !== HISTORYEMPTY ? data.bathCold : prevLV.bathCold;
  currLV.watering =
    data.watering !== HISTORYEMPTY ? data.watering : prevLV.watering;
...
export const saveHistory = (
  ...
  console.log("toHistory in saveHistory ->", toHistory);
...
export const Validation = (dispatch, locale) => {
  ...
  if (profile.kitchenHot) {
    ...
    if (!kitchenHot || !checkVal(kitchenHot)) {
      result = false;
      details += getWarnings(
        `${locale.profile_kitchen}->${locale.profile_kitchen_hot}`
      );
    }
  }
  if (profile.kitchenCold) {
    if (!kitchenCold || !checkVal(kitchenCold)) {
      result = false;
      details += getWarnings(
        `${locale.profile_kitchen}->${locale.profile_kitchen_cold}`
      );
    }
  }
  if (profile.bathHot) {
    if (!bathHot || !checkVal(bathHot)) {
      result = false;
      details += getWarnings(
        `${locale.profile_bath}->${locale.profile_bath_hot}`
      );
    }
  }
  if (profile.bathCold) {
    if (!bathCold || !checkVal(bathCold)) {
      result = false;
      details += getWarnings(
        `${locale.profile_bath}->${locale.profile_bath_cold}`
      );
    }
  }
  if (profile.watering) {
