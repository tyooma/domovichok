...
export const HistoryDelete = (profileID, history, toHistory, lastValue, toLastValue, locale, navigation) => {
  const Execute = (profileID, history, toHistory, lastValue, toLastValue, locale, navigation) => {
    try {
      let newHistory = {};
      ...
      Alert.alert(
        locale.err_main_caption, `${locale.err_history_delete}\n${err}`,
  ...
  Alert.alert(
    locale.info_warning, locale.info_history_delete,
    [ { text: locale.action_ok, onPress: () => Execute(profileID, history, toHistory, lastValue, toLastValue, locale, navigation)},
...
export const HistoryFilter = (history, filter, locale) => {
  ...
  try {
    ...
    Alert.alert(
      locale.err_main_caption, locale.err_history_filter,
      [{ text: locale.action_ok, onPress: () => null }], { cancelable: false },
    );
  }
  return current;
...
export const HistorySort = (history, DESC, locale) => {
  ...
  try {
    ...
    Alert.alert(
      locale.err_main_caption, locale.err_history_sort,
      [{ text: locale.action_ok, onPress: () => null }], { cancelable: false },
    );
  }
  return current;
