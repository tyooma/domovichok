...
export const runProfileSave = (profile, ProfileID, locale, profiles, toProfiles, lastValue, toLastValue, navigation) => {
  ...
  const validation = (profile, profiles, ProfileID, locale) => {
    ...
    if ( profile.kitchenHot || profile.kitchenCold || profile.bathHot || profile.bathCold || 
         profile.watering || profile.sewage ) {
    } else {
      result = false;
  ...
  const ExecuteProfileSave = (profile, profiles, ProfileID, locale, toProfiles, lastValue, toLastValue, navigation) => {
    const validate = validation(profile, profiles, ProfileID, locale);
    if (validate.state) {
      let newProfiles = [];
      if( !ProfileID ) {
        // Add New Profile
        profiles.forEach((item) => { newProfiles.push(item) });
  ...
  if (ProfileID) {
    Alert.alert(
      locale.info_warning, locale.info_profile_save,
      [ { text: locale.action_ok, onPress: () => ExecuteProfileSave(profile, profiles, ProfileID, locale, toProfiles, lastValue, toLastValue, navigation)},
        { text: locale.action_cancel, onPress: () => null }
      ], { cancelable: false },
    );
  } else {
    ExecuteProfileSave(profile, profiles, ProfileID, locale, toProfiles, lastValue, toLastValue, navigation);
  }
...
export const runProfileDelete = (id, profiles, toProfiles, locale, history, toHistory, lastValue, toLastValue, navigation) => {
  const ExecuteProfileDelete = (id, profiles, toProfiles, locale, history, toHistory, lastValue, toLastValue, navigation) => {
    try {
      let newProfiles = [];
      ...
      Alert.alert(
        locale.err_main_caption, `${locale.err_profile_delete}\n${err}`,
        [{ text: locale.action_ok, onPress: () => null }], { cancelable: false },
      );
    }
  }
  ...
  Alert.alert(
    locale.info_warning, locale.info_profile_delete,
    [ { text: locale.action_ok, onPress: () => ExecuteProfileDelete(id, profiles, toProfiles, locale, history, toHistory, lastValue, toLastValue, navigation)},
      { text: locale.action_cancel, onPress: () => null }
    ], { cancelable: false },
  );
}
